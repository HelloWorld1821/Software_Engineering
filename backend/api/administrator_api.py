...
def get_rooms(db: Session = Depends(get_db)):
    return db.query(db_models.Room).all()
...
def get_records(room_id: int, db: Session = Depends(get_db)):
    return db.query(db_models.Record).filter(db_models.Record.room_id == room_id).all()

#管理员获取特定房间的账单
@router.get("/bills/{room_id}")
def get_bills(room_id: int, db: Session = Depends(get_db)):
    return db.query(db_models.Bill).filter(db_models.Bill.room_id == room_id).order_by(db_models.Bill.bill_id.desc()).first()
...
def create_room(room_id: int, identity_card: str, initial_temperature: float, db: Session = Depends(get_db)):
    ...
    db_room = db_models.Room(room_id=room_id, identity_card=identity_card, initial_temperature=initial_temperature)
    db.add(db_room)
    db_bill = db_models.Bill(room_id=room_id,checkin_time=datetime.now(),total_cost=db_room.total_cost)
    db.add(db_bill)
...
def delete_room(room_id: int, db: Session = Depends(get_db)):
    ...
    db.delete(db_room)
    # 返回最后一个账单
    db_bill = db.query(db_models.Bill).filter(db_models.Bill.room_id == room_id).order_by(db_models.Bill.bill_id.desc()).first()
